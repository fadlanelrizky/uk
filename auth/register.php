...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Check email
    $check = $conn->query("SELECT id_user FROM users WHERE email = '$email'");
    if($check->num_rows > 0) {
        $error = "Email sudah terdaftar!";
    } else {
        $hashed = password_hash($password, PASSWORD_BCRYPT);
        // Daftarkan sebagai role 'user'
        $query = "INSERT INTO users (nama_lengkap, email, password, role) VALUES ('$nama', '$email', '$hashed', 'user')";
        if($conn->query($query)) {
...
<div class="glass-card">
    ...
    <div class="text-center mb-4">
        ...
        <p class="text-muted small">Daftar untuk membeli tiket event favoritmu</p>
    </div>
    ...
    <form action="" method="POST">
        <div class="form-floating mb-3">
            <input type="text" class="form-control" id="floatingNama" name="nama" required placeholder="Nama Lengkap">
